<template>
  <div>
    <div
      class="w-screen leading-10 flex bg-white shadow h-10 z-10 fixed px-4 top-0"
    >
      <svg
        class="w-40 align-middle flex-none"
        viewBox="0 0 159 30"
        fill="none"
      ...
      <div class="flex-grow w-64"></div>
      <div
        class="flex-initial cursor-pointer px-2 border-black transition duration-500 hover:bg-gray-200"
      >
        Home
      </div>
      <div
    ...
    <div
      ...
      v-if="showAdv"
      @click="showAdv = false"
      class="fixed bottom-0 h-24 z-10 w-screen border border-black bg-white"
    >
      Advertiser
    </div>
    <footer class="text-center">
      <div id="category" class="bg-white mx-10 my-16">
        <h1 class="text-4xl border-b border-grey-300 pb-16">Code Formatter</h1>
        <div class="flex flex-wrap flex-row">
          <template v-for="(item, index) in 5">
            <ul :key="`cate_${index}`" class="text-left mx-9 flex-none mt-10">
              <h2 class="text-xl mb-4">Color Converters</h2>
              <li
                class="font-light p-1 cursor-pointer rounded hover:bg-gray-200 transition duration-300"
              >
